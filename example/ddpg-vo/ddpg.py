...
class DDPG:
    def __init__(self, outputs, memorySize, discountFactor,
        ...
        print 'critic'
        self.csv_logger = CSVLogger('critic1.log',append = True)
    ...
    def learnOnMiniBatch(self, miniBatchSize):
        state_batch, action_batch, reward_batch, newState_batch, isFinal_batch, motion_batch \
        ...
        qValuesNewState_batch, premotion_batch = self.critic.target_model.predict_on_batch(critic_input)
        ...
        self.critic.model.fit(X_input, [Y_value,np.array(motion_batch)], validation_split=0.0,
                              batch_size=miniBatchSize, nb_epoch=1, verbose = 0, callbacks=[self.csv_logger])
