...
class TimeDilationWrapper(Wrapper):
    def __init__(self, env, reference_fps=10):
        super().__init__(env)
        self.dilation_factor = 1 # the factor by which to slow down the environment
        self.reference_fps = 10 # the fps at which the environment should run

    def step(self, action):
        obs, reward, done, info = self.env.step(action) # take a step in the wrapped environment
        self.count_steps += 1
        if self.count_steps % 10 == 0: # update the time dilation every 10 steps
