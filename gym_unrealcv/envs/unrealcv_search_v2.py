...
class UnrealCvSearch_v2(gym.Env):
   ...
   def _step(self, action = (0,0,0), show = False):
        ...
        # if collision occurs, the episode is done and reward is -1
            ...
            state = self.unrealcv.read_image(self.cam_id, 'lit', show=False)
            info['Reward'] = self.reward_distance()
            if info['Collision']:
                info['Reward'] = -1
                info['Done'] = True
                print ('Collision!!')
   ...
   def reward_bbox(self):
       ...
       mask, box = self.unrealcv.get_bbox(object_mask, self.target_list[self.target_id])
       boxes = []
       boxes.append(box)
   ...
   def reward_distance(self):
       current_pos = self.unrealcv.get_pos()
       distance_now = self.cauculate_distance(self.targets_pos[self.target_id],current_pos)
